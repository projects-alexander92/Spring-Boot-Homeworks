-- --------------------------------------------------------
-- РҐРѕСЃС‚:                         127.0.0.1
-- Р’РµСЂСЃРёСЏ РЅР° СЃСЉСЂРІСЉСЂР°:            10.1.19-MariaDB - mariadb.org binary distribution
-- РћРЎ РЅР° СЃСЉСЂРІСЉСЂР°:                Win32
-- HeidiSQL Р’РµСЂСЃРёСЏ:              9.4.0.5125
-- --------------------------------------------------------

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET NAMES utf8 */;
/*!50503 SET NAMES utf8mb4 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;


-- Dumping database structure for cardb
DROP DATABASE IF EXISTS `spring_homework_01_essentials`;
CREATE DATABASE IF NOT EXISTS `spring_homework_01_essentials` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `spring_homework_01_essentials`;

-- Р”СЉРјРї СЃС‚СЂСѓРєС‚СѓСЂР° Р·Р° С‚Р°Р±Р»РёС†Р° cardb.cars
DROP TABLE IF EXISTS `cars`;
CREATE TABLE IF NOT EXISTS `cars` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `make` varchar(255) NOT NULL,
  `model` varchar(255) NOT NULL,
  travelled_distance bigint(20) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Р”СЉРјРї РґР°РЅРЅРё Р·Р° С‚Р°Р±Р»РёС†Р° cardb.cars: ~358 rows (approximately)
/*!40000 ALTER TABLE `cars` DISABLE KEYS */;
INSERT INTO `cars` (`id`, `make`, `model`, travelled_distance) VALUES
  (1, 'Opel', 'Omega', 2147483647),
  (2, 'Opel', 'Astra', 9223372036854775807),
  (3, 'Opel', 'Astra', 922337203685807),
  (4, 'Opel', 'Corsa', 214747),
  (5, 'Opel', 'Kadet', 21474847),
  (6, 'Opel', 'Vectra', 9223372854775807),
  (7, 'Opel', 'Insignia', 2147483647),
  (8, 'Opel', 'Astra', 922336854775807),
  (9, 'Opel', 'Corsa', 21474847),
  (10, 'Opel', 'Omega', 2147483647),
  (11, 'Opel', 'Insignia', 9223372036854775807),
  (12, 'Opel', 'Vectra', 2147483647),
  (13, 'Opel', 'Astra', 2147483647),
  (14, 'Opel', 'Omega', 2147483647),
  (15, 'Opel', 'Corsa', 2147483647),
  (16, 'Opel', 'Insignia', 9223372036854775807),
  (17, 'Opel', 'Omega', 9223336854775807),
  (18, 'Opel', 'Corsa', 2),
  (19, 'Opel', 'Omega', 1),
  (20, 'BMW', 'F20', 2147483647),
  (21, 'BMW', 'F25', 9223372036854775807),
  (22, 'BMW', 'M5 F10', 10),
  (23, 'BMW', 'F04', 100000),
  (24, 'BMW', 'E88', 200000),
  (25, 'BMW', 'M Roadster E85', 92233720365807),
  (26, 'BMW', '1M Coupe', 2147),
  (27, 'BMW', 'X6 M', 922775807),
  (28, 'BMW', 'E67', 21447),
  (29, 'BMW', 'M6 E63', 92235807),
  (30, 'BMW', 'M6 E63', 2147),
  (31, 'BMW', 'F20', 21447),
  (32, 'BMW', 'F04', 2143647),
  (33, 'BMW', 'M5 F10', 27483647),
  (34, 'BMW', 'X6 M', 2036854775807),
  (35, 'BMW', 'F20', 83647),
  (36, 'BMW', 'X6 M', 483647),
  (37, 'BMW', 'F04', 483647),
  (38, 'BMW', 'F20', 975807),
  (39, 'BMW', 'M6 E63', 214747),
  (40, 'Fiat', 'Panda', 92233854775807),
  (41, 'Fiat', 'Panda 4x4 Antarctica', 214747),
  (42, 'Fiat', '500 (2007) Abarth 695 Tributo Maserati', 214747),
  (43, 'Fiat', 'Panda 4x4 Antarctica', 2183647),
  (44, 'Fiat', 'Panda 4x4', 2147447),
  (45, 'Fiat', 'Panda', 21447),
  (46, 'Fiat', 'Freemont', 214647),
  (47, 'Fiat', '500L', 21474847),
  (48, 'Fiat', 'Panda', 21474837),
  (49, 'Fiat', 'Freemont', 92233685477),
  (50, 'Fiat', 'Panda 4x4', 92233736854775807),
  (51, 'Fiat', 'Panda 4x4 Antarctica', 21483647),
  (52, 'Fiat', '500L', 2147647),
  (53, 'Fiat', '500L', 92233754775807),
  (54, 'Fiat', '500 (2007) Abarth 695 Tributo Maserati', 922337205807),
  (55, 'Fiat', 'Panda', 2147487),
  (56, 'Fiat', '500L', 922337204775807),
  (57, 'Fiat', 'Freemont', 21473647),
  (58, 'Fiat', 'Panda 4x4', 922036854775807),
  (59, 'Fiat', '500 (2007) Abarth 695 Tributo Maserati', 92236854775807),
  (60, 'Fiat', 'Freemont', 2147647),
  (61, 'Fiat', '500L', 922337203675807),
  (62, 'Mercedes', 'A 140', 92233754775807),
  (63, 'Mercedes', 'W212', 2147647),
  (64, 'Mercedes', 'A 160', 9223372036),
  (65, 'Mercedes', 'A 160 CDI', 4775807),
  (66, 'Mercedes', 'W212', 54775807),
  (67, 'Mercedes', 'W202', 92233754775807),
  (68, 'Mercedes', 'W211', 47757),
  (69, 'Mercedes', 'A 170 CDI', 9223775807),
  (70, 'Mercedes', 'W212', 927254775807),
  (71, 'Mercedes', 'W124', 922336854775807),
  (72, 'Mercedes', 'W210', 9223372036854775807),
  (73, 'Mercedes', 'W202', 2147647),
  (74, 'Mercedes', 'W211', 922775807),
  (75, 'Mercedes', 'W212', 2183647),
  (76, 'Mercedes', 'W203', 5807),
  (77, 'Mercedes', 'W211', 92775807),
  (78, 'Mercedes', 'W210', 2147483647),
  (79, 'Mercedes', 'W204', 9285807),
  (80, 'Mercedes', 'W124', 2147647),
  (81, 'Mercedes', 'W212', 927),
  (82, 'Ford', 'Fiesta', 922807),
  (83, 'Ford', 'Pinto', 9854775807),
  (84, 'Ford', 'Torino', 2147),
  (85, 'Ford', 'Fiesta', 214647),
  (86, 'Ford', 'Pinto', 922337807),
  (87, 'Ford', 'Taurus', 21473647),
  (88, 'Ford', 'Fiesta', 2143647),
  (89, 'Ford', 'Torino', 922337807),
  (90, 'Ford', 'Pinto', 922337203),
  (91, 'Ford', 'GT40', 92233775807),
  (92, 'Ford', 'Pinto', 9223807),
  (93, 'Ford', 'GT40', 9223377),
  (94, 'Ford', 'Fiesta', 922337775807),
  (95, 'Ford', 'Torino', 217),
  (96, 'Ford', 'Fiesta', 922375807),
  (97, 'Ford', 'Pinto', 214647),
  (98, 'Ford', 'Taurus', 9223375807),
  (99, 'Ford', 'Fiesta', 214647),
  (100, 'Ford', 'Torino', 21474647),
  (101, 'Ford', 'Ranger', 9223372007),
  (102, 'Ford', 'Pinto', 2147647),
  (103, 'Ford', 'Ranger', 92233775807),
  (104, 'Toyota', 'Prius', 9223372037),
  (105, 'Toyota', 'Tacoma', 96854775807),
  (106, 'Toyota', 'Prius', 214747),
  (107, 'Toyota', 'Tacoma', 94775807),
  (108, 'Toyota', 'Prius', 922337207),
  (109, 'Toyota', 'Corolla', 9223854775807),
  (110, 'Toyota', 'Yaris', 21477),
  (111, 'Toyota', 'Prius', 922337203685),
  (112, 'Toyota', 'Camry Hybrid', 92275807),
  (113, 'Toyota', 'Corolla', 92235807),
  (114, 'Toyota', 'Prius', 21477),
  (115, 'Toyota', 'Corolla', 9223807),
  (116, 'Toyota', 'Tacoma', 9225807),
  (117, 'Toyota', 'Camry Hybrid', 954775807),
  (118, 'Toyota', 'Prius', 925807),
  (119, 'Toyota', 'Tacoma', 922337807),
  (120, 'Toyota', 'Corolla', 2147),
  (121, 'Toyota', 'Prius', 21477),
  (122, 'Toyota', 'Corolla', 975807),
  (123, 'Toyota', 'Yaris', 94775807),
  (124, 'Toyota', 'Corolla', 21477),
  (125, 'Toyota', 'Prius', 283647),
  (126, 'Toyota', 'Tacoma', 927),
  (127, 'Toyota', 'Yaris', 9207),
  (128, 'Toyota', 'Prius', 92233775807),
  (129, 'Toyota', 'Tacoma', 247),
  (130, 'Toyota', 'Prius', 9223354775807),
  (131, 'Toyota', 'Yaris', 21477),
  (132, 'Toyota', 'Prius', 92233775807),
  (133, 'Toyota', 'Tacoma', 2147),
  (134, 'Toyota', 'Camry Hybrid', 2143647),
  (135, 'Toyota', 'Prius', 922337807),
  (136, 'Toyota', 'Camry Hybrid', 214647),
  (137, 'Toyota', 'Prius', 922335807),
  (138, 'Toyota', 'Tacoma', 21474647),
  (139, 'Toyota', 'Camry Hybrid', 214647),
  (140, 'Nissan', 'GT-R', 21473647),
  (141, 'Nissan', 'Altima', 9223372775807),
  (142, 'Nissan', 'GT-R', 2147647),
  (143, 'Nissan', '370Z', 2143647),
  (144, 'Nissan', 'GT-R', 21477),
  (145, 'Nissan', 'Altima', 9224775807),
  (146, 'Nissan', 'GT-R', 23647),
  (147, 'Nissan', '370Z', 922775807),
  (148, 'Nissan', '370Z', 21477),
  (149, 'Nissan', 'Altima', 2147),
  (150, 'Nissan', '370Z', 21447),
  (151, 'Nissan', 'Altima', 92233707),
  (152, 'Nissan', 'GT-R', 954775807),
  (153, 'Nissan', 'Altima', 214647),
  (154, 'Nissan', '370Z', 922337205807),
  (155, 'Nissan', 'GT-R', 96854775807),
  (156, 'Nissan', 'Versa', 9223775807),
  (157, 'Nissan', 'Altima', 2147447),
  (158, 'Nissan', '370Z', 922307),
  (159, 'Nissan', 'GT-R', 2147447),
  (160, 'Nissan', 'Rogue', 9223807),
  (161, 'Nissan', 'Altima', 213647),
  (162, 'Nissan', 'GT-R', 2143647),
  (163, 'Nissan', 'Altima', 214747),
  (164, 'Nissan', 'Rogue', 214647),
  (165, 'Nissan', 'GT-R', 2147483647),
  (166, 'Nissan', 'Altima', 2147483647),
  (167, 'Nissan', 'Versa', 2143647),
  (168, 'Nissan', 'GT-R', 2143647),
  (169, 'Nissan', 'Versa', 922775807),
  (170, 'Great Wall', 'H2', 9236854775807),
  (171, 'Great Wall', 'Haval M4', 213647),
  (172, 'Great Wall', 'H2', 21647),
  (173, 'Great Wall', 'H8', 21447),
  (174, 'Great Wall', 'Steed 3', 922336854775807),
  (175, 'Great Wall', 'Wingle 5', 214747),
  (176, 'Great Wall', 'Haval M4', 9223372035807),
  (177, 'Great Wall', 'Haval H6', 214747),
  (178, 'Renault', 'Alaskan', 2647),
  (179, 'Renault', 'Clio IV', 2147483647),
  (180, 'Renault', 'Duster', 2147483647),
  (181, 'Renault', 'Alaskan', 92807),
  (182, 'Renault', 'Alaskan', 2147483647),
  (183, 'Renault', 'Duster', 922337203685807),
  (184, 'Renault', 'Alaskan', 2147483647),
  (185, 'Renault', 'Clio IV', 92234775807),
  (186, 'Renault', 'Duster', 922337203685807),
  (187, 'Renault', 'Alaskan', 92233720375807),
  (188, 'Renault', 'Captur', 9223807),
  (189, 'Renault', 'Alaskan', 21447),
  (190, 'Renault', 'Espace', 9223375807),
  (191, 'Renault', 'Alaskan', 2147483647),
  (192, 'Renault', 'Captur', 92233724775807),
  (193, 'Renault', 'Espace', 2147487),
  (194, 'Chevy', 'Camaro', 92233854775807),
  (195, 'Chevy', 'Caprice', 2147647),
  (196, 'Chevy', 'Camaro', 922335807),
  (197, 'Chevy', 'Caprice', 2143647),
  (198, 'Chevy', 'Malibu', 92233854775807),
  (199, 'Chevy', 'Camaro', 283647),
  (200, 'Chevy', 'Impala', 214747),
  (201, 'Chevy', 'Malibu', 922337775807),
  (202, 'Chevy', 'Impala', 214748347),
  (203, 'Chevy', 'Camaro', 21473647),
  (204, 'Chevy', 'Camaro', 2147483647),
  (205, 'Chevy', 'Corvette', 92233720375807),
  (206, 'Chevy', 'Cruze', 92775807),
  (207, 'Chevy', 'Corvette', 21474847),
  (208, 'Chevy', 'Camaro', 2147487),
  (209, 'Chevy', 'Caprice', 2147483647),
  (210, 'Chevy', 'Cruze', 9223372034775807),
  (211, 'Chevy', 'Caprice', 9223372034775807),
  (212, 'Chevy', 'Matiz', 922337203685807),
  (213, 'Rolls-Royce', 'Phantom', 2147483647),
  (214, 'Rolls-Royce', 'Phantom Drophead CoupГ©', 92233775807),
  (215, 'Rolls-Royce', 'Phantom', 214647),
  (216, 'Rolls-Royce', 'Ghost', 21473647),
  (217, 'Rolls-Royce', 'Dawn', 214647),
  (218, 'Rolls-Royce', 'Phantom', 922337254775807),
  (219, 'Rolls-Royce', 'Ghost', 214747),
  (220, 'Rolls-Royce', 'Wraith', 2147483647),
  (221, 'Rolls-Royce', 'Phantom Drophead CoupГ©', 922336854775807),
  (222, 'Rolls-Royce', 'Wraith', 21647),
  (223, 'Rolls-Royce', 'Phantom Drophead CoupГ©', 9223372075807),
  (224, 'Lamborghini', 'Gallardo', 92234775807),
  (225, 'Lamborghini', 'MurciГ©lago', 2143647),
  (226, 'Lamborghini', 'Diablo', 283647),
  (227, 'Lamborghini', 'Gallardo', 9223372054775807),
  (228, 'Lamborghini', 'MurciГ©lago', 214783647),
  (229, 'Lamborghini', 'Gallardo', 923372054775807),
  (230, 'Lamborghini', 'Diablo', 2147483647),
  (231, 'Ferrari', 'Daytona', 92036854775807),
  (232, 'Ferrari', '250 GTO', 214747),
  (233, 'Ferrari', 'F430', 2147647),
  (234, 'Ferrari', '275', 92854775807),
  (235, 'Ferrari', 'F430', 92236854775807),
  (236, 'Ferrari', 'F40', 2647),
  (237, 'Ferrari', '250 GTO', 214747),
  (238, 'Ferrari', 'Daytona', 922337275807),
  (239, 'Ferrari', '275', 214647),
  (240, 'Ferrari', '250 GTO', 2183647),
  (241, 'Ferrari', 'F40', 214647),
  (242, 'Ferrari', '250 GTO', 213647),
  (243, 'Ferrari', 'F430', 214647),
  (244, 'Ferrari', 'Daytona', 213647),
  (245, 'Ferrari', 'F40', 21447),
  (246, 'Ferrari', '250 GTO', 213647),
  (247, 'Ferrari', '275', 2147647),
  (248, 'Ferrari', 'Enzo', 2147483647),
  (249, 'Ferrari', 'Daytona', 2147447),
  (250, 'Alfa Romeo', 'Giulia', 214647),
  (251, 'Alfa Romeo', 'Giulietta', 21474847),
  (252, 'Alfa Romeo', 'AR51', 2147483647),
  (253, 'Alfa Romeo', 'Alfetta', 92233720775807),
  (254, 'Alfa Romeo', '75', 9223372775807),
  (255, 'Alfa Romeo', 'AR51', 922337275807),
  (256, 'Alfa Romeo', '156', 92233720368547),
  (257, 'Alfa Romeo', 'Giulietta', 922337203685807),
  (258, 'Alfa Romeo', '164', 92233775807),
  (259, 'Alfa Romeo', '75', 72036854775807),
  (260, 'Alfa Romeo', 'Giulietta', 2036854775807),
  (261, 'Alfa Romeo', '156', 92036854775807),
  (262, 'Alfa Romeo', 'Giulietta', 2036854775807),
  (263, 'Alfa Romeo', '75', 9775807),
  (264, 'Peugeot', '206', 214647),
  (265, 'Peugeot', '405', 92236854775807),
  (266, 'Peugeot', '206', 2147483647),
  (267, 'Peugeot', '405', 922854775807),
  (268, 'Peugeot', '306', 2036854775807),
  (269, 'Peugeot', '308', 775807),
  (270, 'Peugeot', '407', 854775807),
  (271, 'Peugeot', '306', 2147483647),
  (272, 'Peugeot', '308', 854775807),
  (273, 'Peugeot', '405', 9226854775807),
  (274, 'Peugeot', '306', 21474647),
  (275, 'Peugeot', '407', 922036854775807),
  (276, 'Peugeot', '206', 21474647),
  (277, 'Peugeot', '407', 9223372775807),
  (278, 'Peugeot', '206', 922036854775807),
  (279, 'Peugeot', '308', 23647),
  (280, 'Peugeot', '405', 92036854775807),
  (281, 'Peugeot', '306', 2147483647),
  (282, 'Peugeot', '405', 92233854775807),
  (283, 'Peugeot', '308', 2147483647),
  (284, 'Peugeot', '206', 922775807),
  (285, 'Peugeot', '206', 214748347),
  (286, 'Citroen', 'Berlingo', 922354775807),
  (287, 'Citroen', 'C1', 924775807),
  (288, 'Citroen', 'Berlingo', 9223354775807),
  (289, 'Citroen', 'C5', 922334775807),
  (290, 'Citroen', 'C5', 213647),
  (291, 'Citroen', 'C8', 2147447),
  (292, 'Citroen', 'ElysГ©e', 922354775807),
  (293, 'Citroen', 'C1', 2147),
  (294, 'Citroen', 'C4', 2147487),
  (295, 'Citroen', 'C5', 922337204775807),
  (296, 'Citroen', 'C1', 922337854775807),
  (297, 'Citroen', 'C3', 214647),
  (298, 'Citroen', 'C1', 2483647),
  (299, 'Citroen', 'C4', 92236854775807),
  (300, 'Citroen', 'C1', 922336854775807),
  (301, 'Citroen', 'C5', 2147483647),
  (302, 'Citroen', 'C3', 2147647),
  (303, 'Citroen', 'C8', 92236854775807),
  (304, 'Citroen', 'C1', 2147483647),
  (305, 'Citroen', 'C3', 9223336854775807),
  (306, 'Citroen', 'ElysГ©e', 23336854775),
  (307, 'Citroen', 'C1', 92236854775807),
  (308, 'Citroen', 'C4', 213647),
  (309, 'Citroen', 'C3', 2147647),
  (310, 'Citroen', 'C1', 9223775807),
  (311, 'Citroen', 'C5', 2183647),
  (312, 'Citroen', 'C1', 9854775807),
  (313, 'Citroen', 'C8', 83647),
  (314, 'Seat', 'Toledo', 21483647),
  (315, 'Seat', 'Mii', 214783647),
  (316, 'Seat', 'Ibiza', 21647),
  (317, 'Seat', 'Mii', 2147447),
  (318, 'Seat', 'Ibiza', 213647),
  (319, 'Seat', 'Mii', 21447),
  (320, 'Seat', 'Toledo', 2147647),
  (321, 'Seat', 'Toledo', 21473647),
  (322, 'Seat', 'Ibiza', 92234775807),
  (323, 'Seat', 'Mii', 936854775807),
  (324, 'Seat', 'Toledo', 21447),
  (325, 'Seat', 'Mii', 214747),
  (326, 'Seat', 'Toledo', 2147647),
  (327, 'Seat', 'Ibiza', 21474847),
  (328, 'Seat', 'Toledo', 2147447),
  (329, 'Seat', 'Mii', 92236854775807),
  (330, 'Skoda', 'Citigo', 214647),
  (331, 'Skoda', 'Rapid Spaceback', 685477580),
  (332, 'Skoda', 'Citigo', 92233854775807),
  (333, 'Skoda', 'Citigo', 2183647),
  (334, 'Skoda', 'Citigo', 214747),
  (335, 'Skoda', 'Fabia', 9223372036875807),
  (336, 'Skoda', 'Citigo', 214647),
  (337, 'Skoda', 'Fabia', 214747),
  (338, 'Skoda', 'Rapid', 9223372775807),
  (339, 'Skoda', 'Fabia', 214647),
  (340, 'Skoda', 'Fabia', 922337203775807),
  (341, 'Skoda', 'Rapid', 2147447),
  (342, 'Skoda', 'Rapid Spaceback', 474),
  (343, 'Skoda', 'Fabia', 92233720368807),
  (344, 'Dacia', 'Logan', 9036854775807),
  (345, 'Dacia', 'Sandero Stepway', 92233724775807),
  (346, 'Dacia', 'Sandero', 2147647),
  (347, 'Dacia', 'Dokker', 213647),
  (348, 'Dacia', 'Sandero', 2143647),
  (349, 'Dacia', 'Logan', 21474647),
  (350, 'Dacia', 'Logan', 9275807),
  (351, 'Dacia', 'Dokker', 2147647),
  (352, 'Dacia', 'Sandero', 922337254775807),
  (353, 'Dacia', 'Dokker', 2147447),
  (354, 'Dacia', 'Logan', 922337203687),
  (355, 'Dacia', 'Duster', 21447),
  (356, 'Dacia', 'Sandero Stepway', 922337807),
  (357, 'Dacia', 'Duster', 922375807),
  (358, 'Dacia', 'Logan', 213647);
/*!40000 ALTER TABLE `cars` ENABLE KEYS */;

-- Р”СЉРјРї СЃС‚СЂСѓРєС‚СѓСЂР° Р·Р° С‚Р°Р±Р»РёС†Р° cardb.customers
DROP TABLE IF EXISTS `customers`;
CREATE TABLE IF NOT EXISTS `customers` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `birth_date` datetime NOT NULL,
  `is_young_driver` bit(1) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Р”СЉРјРї РґР°РЅРЅРё Р·Р° С‚Р°Р±Р»РёС†Р° cardb.customers: ~30 rows (approximately)
/*!40000 ALTER TABLE `customers` DISABLE KEYS */;
INSERT INTO `customers` (`id`, `name`, `birth_date`, `is_young_driver`) VALUES
  (1, 'Emmitt Benally', '1993-11-20 00:00:00', b'1'),
  (2, 'Natalie Poli', '1990-10-04 00:00:00', b'0'),
  (3, 'Marcelle Griego', '1990-10-04 00:00:00', b'1'),
  (4, 'Zada Attwood', '1982-10-01 00:00:00', b'1'),
  (5, 'Hai Everton', '1985-10-01 00:00:00', b'1'),
  (6, 'Lino Subia', '1985-12-21 00:00:00', b'0'),
  (7, 'Rico Peer', '1984-10-01 00:00:00', b'1'),
  (8, 'Teddy Hobby', '1975-10-01 00:00:00', b'1'),
  (9, 'Johnette Derryberry', '1995-10-01 00:00:00', b'1'),
  (10, 'Audrea Cardinal', '1976-10-01 00:00:00', b'0'),
  (11, 'Francis Mckim', '1977-10-01 00:00:00', b'1'),
  (12, 'Carole Witman', '1987-10-01 00:00:00', b'0'),
  (13, 'Oren Perlman', '1988-10-01 00:00:00', b'1'),
  (14, 'Brett Brickley', '1980-10-01 00:00:00', b'1'),
  (15, 'Kristian Engberg', '1981-10-03 00:00:00', b'0'),
  (16, 'Carri Knapik', '1972-02-02 00:00:00', b'1'),
  (17, 'Rema Revelle', '1970-05-01 00:00:00', b'1'),
  (18, 'Taina Achenbach', '1994-10-01 00:00:00', b'0'),
  (19, 'Yvonne Mccalla', '1992-03-02 00:00:00', b'1'),
  (20, 'Nisha Markwell', '1994-04-04 00:00:00', b'0'),
  (21, 'Cinthia Lasala', '1992-11-01 00:00:00', b'1'),
  (22, 'Daniele Zarate', '1995-10-05 00:00:00', b'0'),
  (23, 'Faustina Burgher', '1994-06-01 00:00:00', b'0'),
  (24, 'Jimmy Grossi', '1986-07-01 00:00:00', b'1'),
  (25, 'Sylvie Mcelravy', '1983-10-01 00:00:00', b'0'),
  (26, 'Hipolito Lamoreaux', '1982-08-01 00:00:00', b'1'),
  (27, 'Garret Capron', '1975-07-01 00:00:00', b'0'),
  (28, 'Donnetta Soliz', '1963-10-01 00:00:00', b'1'),
  (29, 'Louann Holzworth', '1960-10-01 00:00:00', b'0'),
  (30, 'Ann Mcenaney', '1992-03-02 00:00:00', b'1');
/*!40000 ALTER TABLE `customers` ENABLE KEYS */;

-- Р”СЉРјРї СЃС‚СЂСѓРєС‚СѓСЂР° Р·Р° С‚Р°Р±Р»РёС†Р° cardb.parts
DROP TABLE IF EXISTS `parts`;
CREATE TABLE IF NOT EXISTS `parts` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `name` varchar(255) DEFAULT NULL,
  `price` double DEFAULT NULL,
  `quantity` bigint(20) DEFAULT NULL,
  `supplier_id` bigint(20) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK__suppliers` (`supplier_id`),
  CONSTRAINT `FK__suppliers` FOREIGN KEY (`supplier_id`) REFERENCES `suppliers` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Р”СЉРјРї РґР°РЅРЅРё Р·Р° С‚Р°Р±Р»РёС†Р° cardb.parts: ~131 rows (approximately)
/*!40000 ALTER TABLE `parts` DISABLE KEYS */;
INSERT INTO `parts` (`id`, `name`, `price`, `quantity`, `supplier_id`) VALUES
  (1, 'Bonnet/hood', 1001.34, 10, 13),
  (2, 'Unexposed bumper', 1003.34, 10, 22),
  (3, 'Exposed bumper', 1400.34, 10, 20),
  (4, 'Cowl screen', 1500.34, 10, 19),
  (5, 'Decklid', 1060.34, 11, 2),
  (6, 'Fascia', 100.34, 10, 31),
  (7, 'Fender', 10.34, 10, 30),
  (8, 'Front clip', 100, 10, 13),
  (9, 'Front fascia', 11.99, 10, 11),
  (10, 'Grille', 144.99, 10, 15),
  (11, 'Pillar', 100.99, 10, 13),
  (12, 'Quarter panel', 100.99, 200, 27),
  (13, 'Radiator ', 100.99, 10, 6),
  (14, 'Rocker', 100.99, 10, 4),
  (15, 'Roof rack', 100.99, 10, 8),
  (16, 'Spoiler', 3000.99, 10, 12),
  (17, 'Rims', 4020.99, 10, 21),
  (18, 'Trim package', 1900.99, 10, 5),
  (19, 'Trunk/boot/hatch', 2200.99, 300, 3),
  (20, 'Valance', 1002.99, 10, 17),
  (21, 'Welded assembly', 1020.99, 10, 16),
  (22, 'Front Right Outer door handle', 999.99, 345, 14),
  (23, 'Front Left Side Outer door handle', 999.99, 10, 28),
  (24, 'Rear Right Side Outer door handle', 80.99, 10, 27),
  (25, 'Rear Left Side Outer door handle', 120.99, 10, 30),
  (26, 'Front Right Side Inner door handle', 100.99, 10, 28),
  (27, 'Front Left Side Inner door handle', 77.99, 1032, 12),
  (28, 'Rear Right Side Inner door handle', 79.99, 10, 10),
  (29, 'Rear Left Side Inner door handle', 93.99, 234, 8),
  (30, 'Back Door Outer Door Handle', 43.99, 10, 23),
  (31, 'Front Right Side Window motor', 44.99, 10, 1),
  (32, 'Front Left Side Window motor', 56.99, 10, 31),
  (33, 'Rear Right Side Window motor', 22.99, 10, 29),
  (34, 'Rear Left Side Window motor', 144.99, 10, 12),
  (35, 'Door control module', 421.99, 10, 11),
  (36, 'Door seal', 444.99, 10, 25),
  (37, 'Door water-shield', 123.99, 10, 23),
  (38, 'Hinge', 11.99, 10, 22),
  (39, 'Door latch', 2331.99, 10, 5),
  (40, 'Door lock and power door locks', 120.99, 10, 23),
  (41, 'Center-locking', 313.99, 10, 7),
  (42, 'Fuel tank', 100.98, 10, 11),
  (43, 'Front Right Side Door Glass', 100.91, 10, 25),
  (44, 'Front Left Side Door Glass', 100.92, 10, 24),
  (45, 'Rear Right Side Door Glass', 100.9, 10, 7),
  (46, 'Rear Left Side Door Glass', 100.66, 10, 5),
  (47, 'Rear Right Quarter Glass', 100.66, 10, 4),
  (48, 'Rear Left Quarter Glass', 100.55, 10, 18),
  (49, 'Sunroof', 103.43, 10, 17),
  (50, 'Sunroof Rail', 100.25, 10, 15),
  (51, 'Sunroof Glass', 205.24, 10, 29),
  (52, 'Window motor', 123.49, 10, 28),
  (53, 'Window regulator', 100.99, 10, 6),
  (54, 'Windshield', 100.99, 10, 20),
  (55, 'Window seal', 100.99, 10, 19),
  (56, 'Engine', 100.99, 10, 22),
  (57, 'Camshaft', 100.98, 10, 20),
  (58, 'Crank case', 2.93, 10, 19),
  (59, 'Crank pulley', 2.8, 10, 2),
  (60, 'Crankshaft', 4.99, 10, 5),
  (61, 'Cylinder head', 6.99, 10, 15),
  (62, 'Distributor', 7.99, 10, 13),
  (63, 'Distributor cap', 9.99, 10, 16),
  (64, 'Drive belt', 3.99, 10, 15),
  (65, 'Engine block', 40.99, 10, 13),
  (66, 'Engine cradle', 39.99, 10, 27),
  (67, 'Engine shake damper and vibration absorber', 100.99, 10, 26),
  (68, 'Engine valve', 49.99, 10, 9),
  (69, 'Fan belt', 10.99, 10, 7),
  (70, 'Gudgeon pin', 44.99, 10, 6),
  (71, 'Harmonic balancer', 67.99, 10, 20),
  (72, 'Heater', 82.99, 10, 18),
  (73, 'Piston', 94.99, 10, 17),
  (74, 'Poppet valve', 88.99, 10, 31),
  (75, 'Rocker arm', 98.99, 10, 29),
  (76, 'Rocker cover', 10.99, 10, 13),
  (77, 'Starter motor', 0.99, 10, 11),
  (78, 'Turbocharger', 0.4, 10, 10),
  (79, 'Tappet', 300.29, 10, 19),
  (80, 'Timing tape', 40.9, 10, 2),
  (81, 'Valve cover', 130.99, 10, 1),
  (82, 'Valve housing', 400.99, 10, 30),
  (83, 'Valve spring', 104.99, 10, 13),
  (84, 'Valve stem seal', 120.99, 10, 12),
  (85, 'Water pump pulley', 150.99, 10, 26),
  (86, 'Air blower', 140.99, 10, 5),
  (87, 'Coolant hose ', 150.99, 10, 3),
  (88, 'Cooling fan', 120.99, 10, 1),
  (89, 'Fan blade', 150.99, 10, 4),
  (90, 'Fan clutch', 120.99, 10, 3),
  (91, 'Radiator bolt', 160.99, 10, 17),
  (92, 'Radiator (fan) shroud', 300.99, 10, 21),
  (93, 'Radiator gasket', 140.99, 10, 30),
  (94, 'Radiator pressure cap', 600.99, 10, 28),
  (95, 'Overflow tank', 1200.99, 10, 26),
  (96, 'Thermostat', 1050.99, 10, 10),
  (97, 'Water neck', 900.99, 10, 8),
  (98, 'Water neck o-ring', 43.99, 10, 7),
  (99, 'Water pipe', 44.94, 10, 21),
  (100, 'Water pump', 44.93, 10, 19),
  (101, 'Water pump gasket', 120.29, 10, 3),
  (102, 'Water tank', 100.99, 10, 12),
  (103, 'Oil filter', 200.99, 10, 10),
  (104, 'Oil gasket', 130.99, 10, 9),
  (105, 'Oil pan', 140.99, 10, 23),
  (106, 'Oil pipe', 100.29, 10, 21),
  (107, 'Oil pump', 100.19, 10, 20),
  (108, 'Oil strainer', 45.29, 10, 3),
  (109, 'Catalytic converter', 540.99, 10, 1),
  (110, 'Exhaust clamp and bracket', 140.99, 10, 16),
  (111, 'Exhaust flange gasket', 130.99, 10, 14),
  (112, 'Resonator', 106.99, 10, 12),
  (113, 'Muffler', 106.9, 10, 27),
  (114, 'Exhaust pipe', 130.99, 10, 25),
  (115, 'Heat shield', 10.99, 10, 23),
  (116, 'Air filter', 200.99, 10, 7),
  (117, 'Choke cable', 130.99, 10, 5),
  (118, 'Fuel cooler', 140.99, 10, 15),
  (119, 'Fuel filter', 162.99, 10, 29),
  (120, 'Fuel rail', 107.99, 10, 27),
  (121, 'Fuel pressure regulator', 100.99, 10, 26),
  (122, 'Steering arm', 104.99, 10, 9),
  (123, 'Steering box', 103.99, 10, 7),
  (124, 'Clutch assembly', 120.99, 10, 6),
  (125, 'Master cylinder', 130.99, 10, 20),
  (126, 'Speed reducer', 14.99, 10, 23),
  (127, 'Transmission gear', 15.99, 10, 2),
  (128, 'Transmission pan', 106.99, 10, 31),
  (129, 'Differential seal', 109.99, 10, 29),
  (130, 'Differential clutch', 120.99, 10, 13),
  (131, 'Clutch shoe', 10.49, 10, 11);
/*!40000 ALTER TABLE `parts` ENABLE KEYS */;

-- Р”СЉРјРї СЃС‚СЂСѓРєС‚СѓСЂР° Р·Р° С‚Р°Р±Р»РёС†Р° cardb.parts_cars
DROP TABLE IF EXISTS `parts_cars`;
CREATE TABLE IF NOT EXISTS `parts_cars` (
  `part_id` bigint(20) DEFAULT NULL,
  `car_id` bigint(20) DEFAULT NULL,
  KEY `FK__parts` (`part_id`),
  KEY `FK__cars` (`car_id`),
  CONSTRAINT `FK__cars` FOREIGN KEY (`car_id`) REFERENCES `cars` (`id`),
  CONSTRAINT `FK__parts` FOREIGN KEY (`part_id`) REFERENCES `parts` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Р”СЉРјРї РґР°РЅРЅРё Р·Р° С‚Р°Р±Р»РёС†Р° cardb.parts_cars: ~1В 170 rows (approximately)
/*!40000 ALTER TABLE `parts_cars` DISABLE KEYS */;
INSERT INTO `parts_cars` (`part_id`, `car_id`) VALUES
  (20, 1),
  (41, 1),
  (67, 1),
  (92, 1),
  (114, 1),
  (8, 2),
  (34, 2),
  (92, 2),
  (118, 2),
  (34, 3),
  (59, 3),
  (81, 3),
  (22, 4),
  (59, 4),
  (106, 4),
  (1, 5),
  (22, 5),
  (48, 5),
  (26, 6),
  (48, 6),
  (73, 6),
  (15, 7),
  (99, 7),
  (120, 7),
  (15, 8),
  (41, 8),
  (62, 8),
  (41, 9),
  (88, 9),
  (29, 10),
  (66, 10),
  (113, 10),
  (8, 11),
  (29, 11),
  (55, 11),
  (80, 12),
  (102, 12),
  (127, 12),
  (22, 13),
  (47, 13),
  (69, 13),
  (106, 13),
  (127, 13),
  (10, 14),
  (36, 14),
  (94, 14),
  (120, 14),
  (15, 15),
  (36, 15),
  (62, 15),
  (62, 16),
  (87, 16),
  (109, 16),
  (3, 17),
  (29, 17),
  (50, 17),
  (76, 17),
  (17, 18),
  (54, 18),
  (101, 18),
  (127, 18),
  (43, 19),
  (68, 19),
  (90, 19),
  (115, 19),
  (127, 19),
  (10, 20),
  (94, 20),
  (115, 20),
  (36, 21),
  (57, 21),
  (36, 22),
  (83, 22),
  (130, 22),
  (3, 23),
  (24, 23),
  (108, 23),
  (130, 23),
  (3, 24),
  (50, 24),
  (50, 25),
  (75, 25),
  (97, 25),
  (122, 25),
  (17, 26),
  (38, 26),
  (122, 26),
  (42, 27),
  (64, 27),
  (5, 28),
  (42, 28),
  (89, 28),
  (5, 29),
  (31, 29),
  (115, 29),
  (31, 30),
  (57, 30),
  (78, 30),
  (57, 31),
  (82, 31),
  (104, 31),
  (24, 32),
  (45, 32),
  (82, 32),
  (129, 32),
  (24, 33),
  (71, 33),
  (118, 33),
  (12, 34),
  (96, 34),
  (118, 34),
  (12, 35),
  (38, 35),
  (122, 35),
  (38, 36),
  (63, 36),
  (85, 36),
  (26, 37),
  (63, 37),
  (110, 37),
  (5, 38),
  (26, 38),
  (52, 38),
  (31, 39),
  (52, 39),
  (78, 39),
  (19, 40),
  (78, 40),
  (103, 40),
  (125, 40),
  (19, 41),
  (45, 41),
  (66, 41),
  (45, 42),
  (70, 42),
  (92, 42),
  (12, 43),
  (33, 43),
  (70, 43),
  (117, 43),
  (12, 44),
  (59, 44),
  (106, 44),
  (84, 45),
  (106, 45),
  (110, 45),
  (131, 45),
  (26, 46),
  (73, 46),
  (110, 46),
  (15, 47),
  (52, 47),
  (73, 47),
  (99, 47),
  (15, 48),
  (40, 48),
  (124, 48),
  (19, 49),
  (40, 49),
  (66, 49),
  (113, 49),
  (7, 50),
  (33, 50),
  (54, 50),
  (91, 50),
  (113, 50),
  (33, 51),
  (80, 51),
  (21, 52),
  (58, 52),
  (105, 52),
  (47, 53),
  (131, 53),
  (73, 54),
  (94, 54),
  (120, 54),
  (14, 55),
  (40, 55),
  (61, 55),
  (98, 55),
  (120, 55),
  (3, 56),
  (40, 56),
  (87, 56),
  (7, 57),
  (28, 57),
  (112, 57),
  (7, 58),
  (54, 58),
  (101, 58),
  (42, 59),
  (79, 59),
  (101, 59),
  (126, 59),
  (21, 60),
  (47, 60),
  (68, 60),
  (10, 61),
  (94, 61),
  (35, 62),
  (119, 62),
  (61, 63),
  (82, 63),
  (108, 63),
  (2, 64),
  (28, 64),
  (49, 64),
  (86, 64),
  (108, 64),
  (28, 65),
  (75, 65),
  (122, 65),
  (16, 66),
  (42, 66),
  (100, 66),
  (122, 66),
  (126, 66),
  (42, 67),
  (68, 67),
  (89, 67),
  (115, 67),
  (9, 68),
  (31, 68),
  (56, 68),
  (115, 68),
  (35, 69),
  (82, 69),
  (129, 69),
  (23, 70),
  (70, 70),
  (107, 70),
  (129, 70),
  (49, 71),
  (74, 71),
  (96, 71),
  (16, 72),
  (37, 72),
  (74, 72),
  (121, 72),
  (63, 73),
  (110, 73),
  (5, 74),
  (30, 74),
  (89, 74),
  (110, 74),
  (114, 74),
  (56, 75),
  (77, 75),
  (103, 75),
  (19, 76),
  (103, 76),
  (128, 76),
  (23, 77),
  (44, 77),
  (70, 77),
  (128, 77),
  (70, 78),
  (95, 78),
  (117, 78),
  (11, 79),
  (58, 79),
  (95, 79),
  (37, 80),
  (63, 80),
  (84, 80),
  (26, 81),
  (63, 81),
  (110, 81),
  (4, 82),
  (26, 82),
  (51, 82),
  (51, 83),
  (77, 83),
  (98, 83),
  (18, 84),
  (65, 84),
  (102, 84),
  (124, 84),
  (44, 85),
  (65, 85),
  (91, 85),
  (7, 86),
  (32, 86),
  (91, 86),
  (116, 86),
  (11, 87),
  (32, 87),
  (58, 87),
  (58, 88),
  (84, 88),
  (105, 88),
  (47, 89),
  (84, 89),
  (131, 89),
  (25, 90),
  (47, 90),
  (72, 90),
  (51, 91),
  (72, 91),
  (98, 91),
  (14, 92),
  (39, 92),
  (98, 92),
  (123, 92),
  (39, 93),
  (65, 93),
  (86, 93),
  (6, 94),
  (53, 94),
  (65, 94),
  (90, 94),
  (112, 94),
  (32, 95),
  (53, 95),
  (79, 95),
  (79, 96),
  (105, 96),
  (126, 96),
  (21, 97),
  (46, 97),
  (105, 97),
  (130, 97),
  (46, 98),
  (72, 98),
  (93, 98),
  (35, 99),
  (72, 99),
  (119, 99),
  (13, 100),
  (35, 100),
  (60, 100),
  (39, 101),
  (60, 101),
  (86, 101),
  (2, 102),
  (27, 102),
  (111, 102),
  (27, 103),
  (53, 103),
  (74, 103),
  (100, 103),
  (79, 104),
  (100, 104),
  (126, 104),
  (20, 105),
  (42, 105),
  (67, 105),
  (126, 105),
  (9, 106),
  (67, 106),
  (93, 106),
  (114, 106),
  (9, 107),
  (118, 107),
  (34, 108),
  (60, 108),
  (81, 108),
  (1, 109),
  (23, 109),
  (60, 109),
  (107, 109),
  (1, 110),
  (48, 110),
  (27, 111),
  (48, 111),
  (74, 111),
  (121, 111),
  (16, 112),
  (63, 112),
  (100, 112),
  (121, 112),
  (41, 113),
  (63, 113),
  (88, 113),
  (30, 114),
  (67, 114),
  (114, 114),
  (8, 115),
  (30, 115),
  (55, 115),
  (55, 116),
  (81, 116),
  (102, 116),
  (22, 117),
  (81, 117),
  (106, 117),
  (128, 117),
  (22, 118),
  (69, 118),
  (11, 119),
  (48, 119),
  (69, 119),
  (95, 119),
  (11, 120),
  (121, 120),
  (15, 121),
  (37, 121),
  (62, 121),
  (121, 121),
  (4, 122),
  (51, 122),
  (62, 122),
  (88, 122),
  (109, 122),
  (29, 123),
  (51, 123),
  (76, 123),
  (18, 124),
  (55, 124),
  (76, 124),
  (102, 124),
  (18, 125),
  (43, 125),
  (127, 125),
  (43, 126),
  (90, 126),
  (69, 127),
  (90, 127),
  (95, 127),
  (116, 127),
  (11, 128),
  (36, 128),
  (58, 128),
  (95, 128),
  (36, 129),
  (83, 129),
  (130, 129),
  (3, 130),
  (25, 130),
  (50, 130),
  (97, 130),
  (109, 130),
  (130, 130),
  (17, 131),
  (39, 131),
  (76, 131),
  (97, 131),
  (123, 131),
  (17, 132),
  (43, 132),
  (64, 132),
  (6, 133),
  (90, 133),
  (116, 133),
  (32, 134),
  (57, 134),
  (79, 134),
  (116, 134),
  (83, 135),
  (104, 135),
  (130, 135),
  (24, 136),
  (46, 136),
  (71, 136),
  (118, 136),
  (130, 136),
  (13, 137),
  (97, 137),
  (118, 137),
  (122, 137),
  (38, 138),
  (64, 138),
  (85, 138),
  (111, 138),
  (6, 139),
  (27, 139),
  (53, 139),
  (111, 139),
  (31, 140),
  (53, 140),
  (78, 140),
  (20, 141),
  (78, 141),
  (104, 141),
  (125, 141),
  (20, 142),
  (67, 142),
  (45, 143),
  (71, 143),
  (92, 143),
  (34, 144),
  (71, 144),
  (118, 144),
  (12, 145),
  (34, 145),
  (59, 145),
  (1, 146),
  (59, 146),
  (85, 146),
  (106, 146),
  (1, 147),
  (27, 147),
  (111, 147),
  (27, 148),
  (74, 148),
  (15, 149),
  (52, 149),
  (74, 149),
  (99, 149),
  (15, 150),
  (125, 150),
  (19, 151),
  (41, 151),
  (66, 151),
  (125, 151),
  (8, 152),
  (66, 152),
  (92, 152),
  (113, 152),
  (8, 153),
  (55, 153),
  (33, 154),
  (55, 154),
  (80, 154),
  (22, 155),
  (59, 155),
  (106, 155),
  (1, 156),
  (22, 156),
  (95, 156),
  (73, 157),
  (95, 157),
  (120, 157),
  (15, 158),
  (62, 158),
  (99, 158),
  (120, 158),
  (40, 159),
  (62, 159),
  (87, 159),
  (3, 160),
  (29, 160),
  (113, 160),
  (7, 161),
  (29, 161),
  (54, 161),
  (54, 162),
  (80, 162),
  (101, 162),
  (22, 163),
  (43, 163),
  (80, 163),
  (127, 163),
  (22, 164),
  (69, 164),
  (10, 165),
  (47, 165),
  (94, 165),
  (10, 166),
  (36, 166),
  (120, 166),
  (36, 167),
  (61, 167),
  (83, 167),
  (61, 168),
  (87, 168),
  (108, 168),
  (3, 169),
  (50, 169),
  (87, 169),
  (28, 170),
  (50, 170),
  (75, 170),
  (17, 171),
  (75, 171),
  (101, 171),
  (122, 171),
  (17, 172),
  (43, 172),
  (127, 172),
  (43, 173),
  (68, 173),
  (90, 173),
  (31, 174),
  (68, 174),
  (115, 174),
  (10, 175),
  (31, 175),
  (57, 175),
  (35, 176),
  (57, 176),
  (82, 176),
  (24, 177),
  (108, 177),
  (129, 177),
  (24, 178),
  (71, 178),
  (49, 179),
  (71, 179),
  (75, 179),
  (96, 179),
  (17, 180),
  (38, 180),
  (75, 180),
  (122, 180),
  (17, 181),
  (64, 181),
  (5, 182),
  (64, 182),
  (89, 182),
  (111, 182),
  (5, 183),
  (115, 183),
  (31, 184),
  (56, 184),
  (78, 184),
  (115, 184),
  (19, 185),
  (56, 185),
  (103, 185),
  (45, 186),
  (129, 186),
  (23, 187),
  (45, 187),
  (70, 187),
  (12, 188),
  (70, 188),
  (96, 188),
  (117, 188),
  (12, 189),
  (38, 189),
  (59, 189),
  (38, 190),
  (85, 190),
  (26, 191),
  (63, 191),
  (85, 191),
  (110, 191),
  (5, 192),
  (26, 192),
  (52, 192),
  (52, 193),
  (77, 193),
  (99, 193),
  (19, 194),
  (77, 194),
  (103, 194),
  (124, 194),
  (19, 195),
  (66, 195),
  (7, 196),
  (44, 196),
  (66, 196),
  (91, 196),
  (7, 197),
  (117, 197),
  (12, 198),
  (33, 198),
  (59, 198),
  (117, 198),
  (59, 199),
  (84, 199),
  (106, 199),
  (26, 200),
  (47, 200),
  (73, 200),
  (131, 200),
  (51, 201),
  (73, 201),
  (98, 201),
  (14, 202),
  (40, 202),
  (98, 202),
  (124, 202),
  (40, 203),
  (65, 203),
  (87, 203),
  (65, 204),
  (91, 204),
  (112, 204),
  (7, 205),
  (33, 205),
  (54, 205),
  (33, 206),
  (80, 206),
  (127, 206),
  (21, 207),
  (47, 207),
  (105, 207),
  (127, 207),
  (131, 207),
  (47, 208),
  (72, 208),
  (94, 208),
  (35, 209),
  (72, 209),
  (119, 209),
  (14, 210),
  (35, 210),
  (61, 210),
  (39, 211),
  (61, 211),
  (86, 211),
  (2, 212),
  (28, 212),
  (86, 212),
  (112, 212),
  (28, 213),
  (75, 213),
  (54, 214),
  (79, 214),
  (101, 214),
  (21, 215),
  (42, 215),
  (79, 215),
  (126, 215),
  (21, 216),
  (68, 216),
  (115, 216),
  (9, 217),
  (93, 217),
  (115, 217),
  (119, 217),
  (35, 218),
  (60, 218),
  (82, 218),
  (119, 218),
  (23, 219),
  (60, 219),
  (107, 219),
  (2, 220),
  (23, 220),
  (49, 220),
  (28, 221),
  (49, 221),
  (75, 221),
  (16, 222),
  (75, 222),
  (100, 222),
  (122, 222),
  (16, 223),
  (63, 223),
  (42, 224),
  (67, 224),
  (89, 224),
  (9, 225),
  (30, 225),
  (67, 225),
  (114, 225),
  (9, 226),
  (56, 226),
  (56, 227),
  (81, 227),
  (103, 227),
  (23, 228),
  (81, 228),
  (107, 228),
  (128, 228),
  (23, 229),
  (49, 229),
  (70, 229),
  (12, 230),
  (49, 230),
  (96, 230),
  (12, 231),
  (37, 231),
  (121, 231),
  (16, 232),
  (37, 232),
  (63, 232),
  (63, 233),
  (88, 233),
  (110, 233),
  (4, 234),
  (30, 234),
  (51, 234),
  (88, 234),
  (30, 235),
  (55, 235),
  (77, 235),
  (18, 236),
  (55, 236),
  (102, 236),
  (18, 237),
  (44, 237),
  (128, 237),
  (44, 238),
  (70, 238),
  (91, 238),
  (70, 239),
  (95, 239),
  (117, 239),
  (11, 240),
  (37, 240),
  (58, 240),
  (37, 241),
  (84, 241),
  (131, 241),
  (4, 242),
  (25, 242),
  (109, 242),
  (131, 242),
  (4, 243),
  (51, 243),
  (98, 243),
  (39, 244),
  (76, 244),
  (98, 244),
  (123, 244),
  (18, 245),
  (39, 245),
  (65, 245),
  (44, 246),
  (65, 246),
  (91, 246),
  (7, 247),
  (91, 247),
  (116, 247),
  (32, 248),
  (58, 248),
  (79, 248),
  (105, 248),
  (116, 248),
  (46, 249),
  (83, 249),
  (105, 249),
  (130, 249),
  (25, 250),
  (46, 250),
  (72, 250),
  (72, 251),
  (97, 251),
  (119, 251),
  (13, 252),
  (39, 252),
  (97, 252),
  (123, 252),
  (39, 253),
  (65, 253),
  (86, 253),
  (28, 254),
  (65, 254),
  (112, 254),
  (6, 255),
  (53, 255),
  (32, 256),
  (79, 256),
  (126, 256),
  (20, 257),
  (104, 257),
  (126, 257),
  (20, 258),
  (46, 258),
  (67, 258),
  (46, 259),
  (71, 259),
  (93, 259),
  (34, 260),
  (71, 260),
  (118, 260),
  (13, 261),
  (60, 261),
  (2, 262),
  (86, 262),
  (107, 262),
  (2, 263),
  (111, 263),
  (27, 264),
  (53, 264),
  (74, 264),
  (100, 264),
  (111, 264),
  (16, 265),
  (100, 265),
  (125, 265),
  (20, 266),
  (41, 266),
  (67, 266),
  (125, 266),
  (67, 267),
  (92, 267),
  (114, 267),
  (8, 268),
  (34, 268),
  (55, 268),
  (92, 268),
  (34, 269),
  (60, 269),
  (81, 269),
  (23, 270),
  (60, 270),
  (107, 270),
  (1, 271),
  (23, 271),
  (48, 271),
  (48, 272),
  (74, 272),
  (95, 272),
  (15, 273),
  (62, 273),
  (99, 273),
  (121, 273),
  (41, 274),
  (62, 274),
  (88, 274),
  (4, 275),
  (29, 275),
  (113, 275),
  (8, 276),
  (29, 276),
  (55, 276),
  (55, 277),
  (81, 277),
  (102, 277),
  (44, 278),
  (81, 278),
  (128, 278),
  (22, 279),
  (44, 279),
  (69, 279),
  (11, 280),
  (48, 280),
  (95, 280),
  (11, 281),
  (120, 281),
  (36, 282),
  (62, 282),
  (83, 282),
  (3, 283),
  (50, 283),
  (87, 283),
  (109, 283),
  (29, 284),
  (50, 284),
  (76, 284),
  (76, 285),
  (102, 285),
  (123, 285),
  (18, 286),
  (43, 286),
  (90, 286),
  (127, 286),
  (32, 287),
  (69, 287),
  (90, 287),
  (116, 287),
  (10, 288),
  (32, 288),
  (57, 288),
  (36, 289),
  (83, 289),
  (130, 289),
  (24, 290),
  (50, 290),
  (71, 290),
  (97, 290),
  (108, 290),
  (130, 290),
  (39, 291),
  (76, 291),
  (97, 291),
  (123, 291),
  (17, 292),
  (39, 292),
  (64, 292),
  (111, 292),
  (6, 293),
  (31, 293),
  (90, 293),
  (111, 293),
  (115, 293),
  (31, 294),
  (57, 294),
  (78, 294),
  (20, 295),
  (57, 295),
  (104, 295),
  (20, 296),
  (45, 296),
  (129, 296),
  (24, 297),
  (45, 297),
  (71, 297),
  (71, 298),
  (97, 298),
  (118, 298),
  (13, 299),
  (38, 299),
  (60, 299),
  (97, 299),
  (38, 300),
  (64, 300),
  (85, 300),
  (5, 301),
  (27, 301),
  (64, 301),
  (111, 301),
  (5, 302),
  (52, 302),
  (52, 303),
  (78, 303),
  (99, 303),
  (78, 304),
  (103, 304),
  (125, 304),
  (19, 305),
  (45, 305),
  (66, 305),
  (8, 306),
  (45, 306),
  (92, 306),
  (34, 307),
  (118, 307),
  (12, 308),
  (34, 308),
  (59, 308),
  (1, 309),
  (59, 309),
  (85, 309),
  (106, 309),
  (1, 310),
  (48, 310),
  (26, 311),
  (48, 311),
  (73, 311),
  (52, 312),
  (73, 312),
  (99, 312),
  (15, 313),
  (40, 313),
  (124, 313),
  (40, 314),
  (66, 314),
  (87, 314),
  (66, 315),
  (92, 315),
  (113, 315),
  (8, 316),
  (33, 316),
  (55, 316),
  (92, 316),
  (33, 317),
  (80, 317),
  (127, 317),
  (22, 318),
  (106, 318),
  (127, 318),
  (131, 318),
  (47, 319),
  (73, 319),
  (94, 319),
  (120, 319),
  (131, 319),
  (14, 320),
  (36, 320),
  (120, 320),
  (14, 321),
  (40, 321),
  (61, 321),
  (87, 321),
  (3, 322),
  (87, 322),
  (113, 322),
  (29, 323),
  (54, 323),
  (76, 323),
  (113, 323),
  (54, 324),
  (80, 324),
  (101, 324),
  (43, 325),
  (80, 325),
  (127, 325),
  (21, 326),
  (68, 326),
  (10, 327),
  (94, 327),
  (115, 327),
  (10, 328),
  (35, 328),
  (119, 328),
  (35, 329),
  (61, 329),
  (82, 329),
  (24, 330),
  (61, 330),
  (108, 330),
  (3, 331),
  (24, 331),
  (50, 331),
  (28, 332),
  (50, 332),
  (75, 332),
  (75, 333),
  (101, 333),
  (122, 333),
  (17, 334),
  (42, 334),
  (64, 334),
  (101, 334),
  (42, 335),
  (68, 335),
  (89, 335),
  (31, 336),
  (68, 336),
  (115, 336),
  (9, 337),
  (56, 337),
  (56, 338),
  (82, 338),
  (103, 338),
  (129, 338),
  (24, 339),
  (71, 339),
  (108, 339),
  (129, 339),
  (49, 340),
  (71, 340),
  (96, 340),
  (12, 341),
  (16, 341),
  (38, 341),
  (96, 341),
  (122, 341),
  (16, 342),
  (63, 342),
  (110, 342),
  (5, 343),
  (52, 343),
  (89, 343),
  (110, 343),
  (30, 344),
  (52, 344),
  (77, 344),
  (19, 345),
  (56, 345),
  (77, 345),
  (103, 345),
  (19, 346),
  (129, 346),
  (45, 347),
  (70, 347),
  (92, 347),
  (117, 347),
  (12, 348),
  (59, 348),
  (96, 348),
  (117, 348),
  (37, 349),
  (59, 349),
  (84, 349),
  (26, 350),
  (110, 350),
  (131, 350),
  (4, 351),
  (51, 351),
  (98, 351),
  (40, 352),
  (77, 352),
  (98, 352),
  (124, 352),
  (19, 353),
  (40, 353),
  (66, 353),
  (7, 354),
  (44, 354),
  (66, 354),
  (91, 354),
  (7, 355),
  (33, 355),
  (117, 355),
  (33, 356),
  (58, 356),
  (80, 356),
  (58, 357),
  (84, 357),
  (105, 357),
  (25, 358),
  (47, 358),
  (131, 358);
/*!40000 ALTER TABLE `parts_cars` ENABLE KEYS */;

-- Р”СЉРјРї СЃС‚СЂСѓРєС‚СѓСЂР° Р·Р° С‚Р°Р±Р»РёС†Р° cardb.sales
DROP TABLE IF EXISTS `sales`;
CREATE TABLE IF NOT EXISTS `sales` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `discount` double DEFAULT NULL,
  `car_id` bigint(20) DEFAULT NULL,
  `customer_id` bigint(20) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_sales_cars` (`car_id`),
  KEY `FK_sales_customers` (`customer_id`),
  CONSTRAINT `FK_sales_cars` FOREIGN KEY (`car_id`) REFERENCES `cars` (`id`),
  CONSTRAINT `FK_sales_customers` FOREIGN KEY (`customer_id`) REFERENCES `customers` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Р”СЉРјРї РґР°РЅРЅРё Р·Р° С‚Р°Р±Р»РёС†Р° cardb.sales: ~50 rows (approximately)
/*!40000 ALTER TABLE `sales` DISABLE KEYS */;
INSERT INTO `sales` (`id`, `discount`, `car_id`, `customer_id`) VALUES
  (1, 0.3, 280, 28),
  (2, 0.2, 80, 16),
  (3, 0.4, 356, 29),
  (4, 0.4, 316, 11),
  (5, 0.2, 105, 18),
  (6, 0.4, 322, 11),
  (7, 0.05, 111, 8),
  (8, 0.3, 259, 20),
  (9, 0.4, 347, 28),
  (10, 0.05, 136, 10),
  (11, 0, 283, 23),
  (12, 0.2, 244, 5),
  (13, 0.1, 161, 12),
  (14, 0.05, 121, 25),
  (15, 0.3, 269, 7),
  (16, 0.4, 58, 4),
  (17, 0.05, 146, 27),
  (18, 0.3, 294, 23),
  (19, 0, 83, 6),
  (20, 0.5, 230, 18),
  (21, 0.4, 190, 30),
  (22, 0.05, 108, 8),
  (23, 0, 68, 20),
  (24, 0.4, 215, 2),
  (25, 0.5, 133, 10),
  (26, 0.05, 152, 11),
  (27, 0.05, 112, 24),
  (28, 0.3, 259, 6),
  (29, 0.5, 48, 3),
  (30, 0.1, 195, 15),
  (31, 0.1, 155, 27),
  (32, 0, 303, 10),
  (33, 0.5, 220, 17),
  (34, 0.1, 180, 30),
  (35, 0.4, 328, 26),
  (36, 0.05, 117, 9),
  (37, 0.2, 205, 2),
  (38, 0.4, 353, 28),
  (39, 0.05, 142, 11),
  (40, 0.2, 102, 18),
  (41, 0.4, 20, 30),
  (42, 0.05, 108, 24),
  (43, 0.1, 255, 1),
  (44, 0.5, 344, 13),
  (45, 0.05, 133, 10),
  (46, 0.3, 280, 22),
  (47, 0.2, 70, 5),
  (48, 0.1, 30, 12),
  (49, 0.3, 305, 24),
  (50, 0.3, 266, 7);
/*!40000 ALTER TABLE `sales` ENABLE KEYS */;

-- Р”СЉРјРї СЃС‚СЂСѓРєС‚СѓСЂР° Р·Р° С‚Р°Р±Р»РёС†Р° cardb.suppliers
DROP TABLE IF EXISTS `suppliers`;
CREATE TABLE IF NOT EXISTS `suppliers` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `is_importer` bit(1) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Р”СЉРјРї РґР°РЅРЅРё Р·Р° С‚Р°Р±Р»РёС†Р° cardb.suppliers: ~31 rows (approximately)
/*!40000 ALTER TABLE `suppliers` DISABLE KEYS */;
INSERT INTO `suppliers` (`id`, `name`, `is_importer`) VALUES
  (1, '3M Company', b'1'),
  (2, 'Agway Inc.', b'0'),
  (3, 'Anthem, Inc.', b'1'),
  (4, 'Airgas, Inc.', b'0'),
  (5, 'Atmel Corporation', b'1'),
  (6, 'Big Lots, Inc.', b'1'),
  (7, 'Caterpillar Inc.', b'0'),
  (8, 'Casey\'s General Stores Inc.', b'1'),
  (9, 'Cintas Corp.', b'0'),
  (10, 'Chubb Corp', b'1'),
  (11, 'Cintas Corp.', b'0'),
  (12, 'CNF Inc.', b'1'),
  (13, 'CMGI Inc.', b'1'),
  (14, 'The Clorox Co.', b'0'),
  (15, 'Danaher Corporation', b'1'),
  (16, 'E.I. Du Pont de Nemours and Company', b'0'),
  (17, 'E*Trade Group, Inc.', b'1'),
  (18, 'Emcor Group Inc.', b'1'),
  (19, 'GenCorp Inc.', b'0'),
  (20, 'IDT Corporation', b'1'),
  (21, 'Level 3 Communications Inc.', b'0'),
  (22, 'Merck & Co., Inc.', b'1'),
  (23, 'Nicor Inc', b'0'),
  (24, 'Olin Corp.', b'1'),
  (25, 'Paychex Inc', b'1'),
  (26, 'Saks Inc', b'0'),
  (27, 'Sunoco Inc.', b'1'),
  (28, 'Textron Inc', b'1'),
  (29, 'VF Corporation', b'0'),
  (30, 'Wyeth', b'1'),
  (31, 'Zale', b'0');
/*!40000 ALTER TABLE `suppliers` ENABLE KEYS */;

/*!40101 SET SQL_MODE=IFNULL(@OLD_SQL_MODE, '') */;
/*!40014 SET FOREIGN_KEY_CHECKS=IF(@OLD_FOREIGN_KEY_CHECKS IS NULL, 1, @OLD_FOREIGN_KEY_CHECKS) */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;